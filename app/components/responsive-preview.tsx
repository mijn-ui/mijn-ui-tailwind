...
const ResponsivePreview = async ({ name, hideCode }: ResponsivePreviewProps) => {
  ...
  if (!template || template.type !== "view") {
    return (
      <div className="flex min-h-64 flex-col items-start justify-center gap-2 rounded-md border p-6 text-sm text-red-500">
        <div>
          <code className="relative rounded bg-muted px-[0.2rem] py-0 font-mono text-sm">{name}</code> not found.
        </div>
        <div>
  ...
  const htmlContent = getHTMLContent(template.filePath, true)
  const url = `/view/${name}`
  ...
  return (
    <Tabs defaultValue="preview">
      ...
      <div
        ...
        style={
          {
            "--responsive-viewer-height": `${template.iframeHeight || 740}px`,
